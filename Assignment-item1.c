...
void scan_word(int occurrence[26]) {
    ...
    while ((c = getchar()) != '\n') {
        if (isalpha(c)) {
            occurrence[toupper(c) - 'A']++; // Increment the occurrence count for the corresponding letter
        }
    }
}
...
bool is_anagram(int occurrence1[26], int occurrence2[26]) {
    for (int i = 0; i < 26; i++) {
        if (occurrence1[i] != occurrence2[i]) { // If the occurrence count of any letter differs, the words are not anagrams
            return false;
        }
    }
    return true; // All letters have the same occurrence count, the words are anagrams
}

// Function to print the occurrences of each letter in a word
void print_occurrences(int occurrence[26]) {
    for (int i = 0; i < 26; i++) {
        if (occurrence[i] > 0) { // If the occurrence count is greater than 0, print the letter and its count
            printf("%c: %d\n", 'A' + i, occurrence[i]);
        }
    }
}
